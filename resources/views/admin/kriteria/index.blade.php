            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="text-end">
                            <a href="{{ route('admin.kriteria.detail') }}" class="btn btn-info">Detail ROC</a>
                                ...
                                <table id="example" class="display dataTable" style="min-width: 845px" role="grid"
                                    ...
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting" rowspan="1" colspan="1">Prioritas</th>
                                    ...
                                    <tbody>
                                        @foreach ($kriterias as $index => $kriteria)
                                            <tr role="row">
                                                ...
                                                <td>{{ $kriteria->name }}</td>
                                                <td>{{ $kriteria->type }}</td>
                                                <td>{{ $kriteria->priority }}</td>
                                                <td>{{ $kriteria->value }}</td>
                                                <td>
                                                    <a href="{{ route('admin.kriteria.edit', $kriteria->id) }}"
                                                        class="badge badge-warning">Edit</a>
                                                    <a href="{{ route('admin.kriteria.destroy', $kriteria->id) }}"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="input-topik" name="kriterias_id">
                                @foreach ($kriterias as $item)
                                    <option value="{{ $item->id }}" attrBobot="{{ $item->id }}">
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
